...
const projectionScale = halfCanvasHeight;

const data = { points: [], faces: [], uvs: [], height: 0,  };
...
export function drawFaces() {
	for (const face of data.faces) {
		...
		for (const f of face) {
			...
			const canvasPoint = convertToCanvas(translated, halfCanvasWidth, halfCanvasHeight, projectionScale);

            canvasPoint.y += data.height /2 ;
...
({ points: data.points, faces: data.faces, uvs: data.uvs, height: data.height } = parseObj(await res.text()));
state.sceneImageData = ctx.getImageData(0, 0, config.width, config.height);
